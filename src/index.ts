...
interface Elements {
  ...
  camSel: HTMLSelectElement;
  canvas: HTMLCanvasElement;
  brCtrl: HTMLInputElement;
  ctCtrl: HTMLInputElement;
  ditherChk: HTMLInputElement;
  paletteSel: HTMLSelectElement;
  echoMode: HTMLSelectElement;
  captureBtn: HTMLButtonElement;
  captureDialog: HTMLDialogElement;
  captureImg: HTMLImageElement;
  closeDialog: HTMLButtonElement;
  downloadLink: HTMLAnchorElement;
  gifDuration: HTMLInputElement;
  recordGifBtn: HTMLButtonElement;
  gifDialog: HTMLDialogElement;
  gifImg: HTMLImageElement;
  downloadGifLink: HTMLAnchorElement;
  closeGifDialog: HTMLButtonElement;
  settingsBtn: HTMLButtonElement;
  settingsDialog: HTMLDialogElement;
  closeSettingsDialog: HTMLButtonElement;
  echoDecay: HTMLInputElement;
}

const els: Elements = {
  video: getEl<HTMLVideoElement>("videoFeed"),
  camSel: getEl<HTMLSelectElement>("cameraSelect"),
  canvas: getEl<HTMLCanvasElement>("glcanvas"),
  brCtrl: getEl<HTMLInputElement>("brightness"),
  ctCtrl: getEl<HTMLInputElement>("contrast"),
  ditherChk: getEl<HTMLInputElement>("dither"),
  paletteSel: getEl<HTMLSelectElement>("palette"),
  echoMode: getEl<HTMLSelectElement>("echoMode"),
  captureBtn: getEl<HTMLButtonElement>("captureBtn"),
  captureDialog: getEl<HTMLDialogElement>("captureDialog"),
  captureImg: getEl<HTMLImageElement>("captureImg"),
  closeDialog: getEl<HTMLButtonElement>("closeDialog"),
  downloadLink: getEl<HTMLAnchorElement>("downloadLink"),
  gifDuration: getEl<HTMLInputElement>("gifDuration"),
  recordGifBtn: getEl<HTMLButtonElement>("recordGifBtn"),
  gifDialog: getEl<HTMLDialogElement>("gifDialog"),
  gifImg: getEl<HTMLImageElement>("gifImg"),
  downloadGifLink: getEl<HTMLAnchorElement>("downloadGifLink"),
  closeGifDialog: getEl<HTMLButtonElement>("closeGifDialog"),
  settingsBtn: getEl<HTMLButtonElement>("settingsBtn"),
  settingsDialog: getEl<HTMLDialogElement>("settingsDialog"),
  closeSettingsDialog: getEl<HTMLButtonElement>("closeSettingsDialog"),
  echoDecay: getEl<HTMLInputElement>("echoDecay"),
};

// Load GIF worker script into a blob URL (to avoid cross-origin worker loading)
let gifWorkerBlobUrl: string | null;
...
els.camSel.addEventListener("change", async () => {
  await startStream({
    video: { deviceId: { exact: els.camSel.value } },
    audio: false,
  });
});
...
if (currentStream) {
  // FIXME: Type Assersion
  const id = (currentStream as MediaStream)
    .getVideoTracks()[0]
...
const data = await loadPresets("https://2b3d4f.github.io/gbcam-ts/presets.json");
...
// Shader compile and program helpers imported from webgl.ts
...
els.ditherChk.addEventListener("change", () => {
  gl.useProgram(prog);
  gl.uniform1i(locs.useDither, els.ditherChk.checked ? 1 : 0);
});
...
["loadedmetadata", "resize", "playing"].forEach((evt) =>
  els.video.addEventListener(evt, updateCrop)
);
...
const render = (now = 0) => {
  ...
  if (
    textureInitialized &&
    els.video.readyState >= els.video.HAVE_CURRENT_DATA
  ) {
  ...
  const recordInterval = setInterval(() => {
    ...
    if (frameCount >= totalFrames) {
      ...
      gif.on("finished", (blob: Blob) => {
        ...
        els.gifDialog.addEventListener(
          "close",
          () => {
